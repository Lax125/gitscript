...
def preprocess_file(
    ...
    stack: list[Path] = [] if stack is None else list(stack)
    if path in stack:
...
def preprocess_source(source: str, base_dir: str | Path | None = None, stack: list[Path] | None = None) -> str:
    base = (Path.cwd() if base_dir is None else Path(base_dir)).resolve()
    stack = [] if stack is None else list(stack)
    lines = source.splitlines()
    output: list[str] = []

    for line_number, line in enumerate(lines, start=1):
        worktree_path = _worktree_path(line, line_number)
        ...
        if not path.is_absolute():
            path = base / path
        cloned_source, _ = preprocess_file(str(path), stack, base)
...
def _worktree_config(path: Path) -> str:
    return f'git config core.worktree "{_path_text(path)}"'
...
def _worktree_path(line: str, line_number: int) -> str | None:
    stripped = strip_comment(line).strip()
    if not stripped:
        return None
    if not stripped.startswith("git config core.worktree"):
        return None
    ...
    if len(tokens) < 3 or tokens[0].kind != TokenKind.GIT or tokens[1].kind != TokenKind.CONFIG:
        return None
    if tokens[2].value != "core.worktree":
        return None
    if len(tokens) != 4:
        raise PreprocessError(f"Line {line_number}: git config core.worktree expects exactly one path")
